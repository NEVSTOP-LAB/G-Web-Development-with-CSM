...
> 完整文档：[https://nevstop-lab.github.io/CSM-Wiki/docs/examples/csm-gweb-development.html](https://nevstop-lab.github.io/CSM-Wiki/docs/examples/csm-gweb-development.html)
...
通常在LabVIEW Web Service中，每个 HTTP 接口对应一个接口VI——如果应用有 20 个 POST 方法，就要写 20 个接口VI，维护成本较高。

在 CSM 应用中，由于其虚拟总线的设计，可以只定义一个 `CSM-RunScript` 接口，处理所有的 CSM 脚本请求，这样极大地简化了接口层的开发和维护工作量，同时也使得前端调用更加灵活，任何新的功能都可以通过发送新的 CSM 脚本来实现，而无需修改接口代码。用户原本的 CSM 不需要为了适配 Web Service 而做任何改动，完全不涉及网络访问的 CSM 模块代码可以被调用。
...
1. 参考 [CSM 基本概念](https://nevstop-lab.github.io/CSM-Wiki/docs/basic/concepts.html) 和 [示例程序](https://nevstop-lab.github.io/CSM-Wiki/docs/examples/) 创建 CSM 应用工程
2. 将具体的业务逻辑分别封装为独立的 CSM 模块（每个模块对应一个 `.vi`）
3. 在各模块的状态处理分支中，实现 `API: <操作名>` 格式的接口状态，供外部调用
4. 调试并验证各模块的功能，确保应用在 LabVIEW 中可以正常运行

> **提示**：CSM 模块完全不涉及网络访问，只专注于业务逻辑实现。Web Service 层在后续步骤中独立添加，对已有的 CSM 模块代码无需做任何改动。
...
> **提示**：需要安装 LabVIEW Web Service 功能模块（包含在 NI Application Web Server 中）。
